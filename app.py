...
# 배전도 및 가공방식 리스트 (공통 사용)
ROAST_OPTIONS = ["라이트", "라이트-미디엄", "미디엄", "미디엄-다크", "다크", "이탈리안/프렌치"]
PROCESS_OPTIONS = ["네츄럴", "워시드", "허니프로세싱", "무산소발효", "더블무산소", "이스트발효", "디카페인(SWP/CO2)", "블렌딩"]
...
beans = load_json(BEANS_FILE, {"제이든 시그니처 블렌드": {"배전도": "미디엄-다크", "가공방식": "블렌딩", "노트": "초콜릿"}})
...
with tab1:
    if not beans or not baristas:
        ...
        with st.form("form_qc_entry"):
            ...
            st.markdown(f"**💡 정보:** {b_info.get('배전도','')} | {b_info.get('가공방식','')} | {b_info.get('노트','')}")
...
# [탭 2, 3 로직은 이전과 동일하게 유지 - 중복 ID 및 수치 오류 방지 포함]
with tab2:
    df_day = load_qc_logs()
    if not df_day.empty:
        f1, f2 = st.columns(2)
            ...
            with cr:
                for idx, r in day_df.iterrows():
                    with st.expander(f"{r['바리스타']} ({r['추출시간(s)']}초)"):
                        st.write(f"⚖️ {r['도징량(g)']}g ➡️ {r['추출량(g)']}g")
                        if st.button("🚩 공식 채택", key=f"fix_btn_{idx}"):
                            d_key = str(t_date); today_selected.setdefault(d_key, {})[t_bean] = {"dose": r['도징량(g)'], "yield": r['추출량(g)'], "time": r['추출시간(s)'], "by": r['바리스타']}
...
with tab4:
    ...
    with m2:
        ...
        with st.form("f_bean"):
            ...
            # ⭐ 배전도 세분화 적용
            r_idx = ROAST_OPTIONS.index(cur.get("배전도")) if cur.get("배전도") in ROAST_OPTIONS else 2
            ro = c_ro.selectbox("배전도 선택", ROAST_OPTIONS, index=r_idx)
            
            # ⭐ 가공방식 디카페인 추가 적용
            p_idx = PROCESS_OPTIONS.index(cur.get("가공방식")) if cur.get("가공방식") in PROCESS_OPTIONS else 1
            pr = c_pr.selectbox("가공방식 선택", PROCESS_OPTIONS, index=p_idx)
